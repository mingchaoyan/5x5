...
promptForTile2 = (x1, y1) ->
  ...
  inputCallback = (input) ->
    ...
    if x1 is x2 and y1 is y2
      console.log "The second tile must be different from then first"
    else
      ...
      unless moveScore is 0
        ...
        score += moveScore
      moveCount++
      console.log "Your score after #{moveCount} move: #{score}"
      promptForTile1()
...
strToCoordinates = (input) ->
  havles = input.split(',')
  if havles.length is 2
    x = parseFloat havles[0]
    console.log x
    y = parseFloat havles[1]
    console.log y
...
unless usedWords.length is 0
  console.log """
    ...
    #{usedWords.join(', ')}
    """
console.log "Please choose a tile in then form (x, y)."
#e 
